...
const Otherprojects = () => {
  ...
  return (
    <div className="container mx-auto w-full flex flex-col h-auto mt-[150px] items-center justify-center  max-sm:mt-[100px]">
      ...
      <div
        className={`grid grid-cols-3 max-sm:grid max-sm:grid-cols-1 max-sm:w-auto gap-y-6 p-2 w-[1100px] ${length} overflow-hidden mb-8`}
      >
          ...
          return (
            <div className="bg-[#595C77] w-[350px] h-[350px] p-4 rounded-xl max-sm:h-[360px]">
              ...
              <div className="flex w-auto text-[18px] text-[#FFFFFF]  mt-6">
                {tech.map(({ title }) => {
                  return <h4 className="mr-2 hover:text-[#DEBCEE]">{title}</h4>;
                })}
              </div>
            </div>
          );
      ...
      <div>
        <button
          className="w-[200px] text-[#DEBCEE] h-[40px] border border-[#58385E] font-Roboto text-[20px] rounded hover:text-white hover:shadow hover:shadow-sky-400 "
